...
var EasyWeather = {
    Keys: {
        wwo: { key: function() { return 'bead8a07087c44ecafa74545162209'; } },
        owm: { key: function() { return '744656737fc5d66a60ca7520d041b5bd'; } },
        wug: { key: function() { return '52e26b554486f2c0'; } },
        ham: { key: function() { return 'XhRIIwcXCkuN4GWsFCPbh'; }, secret_key: function() { return 'uWSsO1PkEBusaq90cTbASEE8hbytwzg8TBaZX6IG'; } },
        fio: { key: function() { return 'YOUR KEY GOES HERE'; } }
    }
};
    ...
    $.fn.EasyWeather = function (opts) {
        ...
        function locate() {
            ...
            if (loadData) {
                ...
                $.ajax({
                    url: dataUrl,
                    dataType: this.geoProvider().data_type || 'json',
                    context: this,
                    timeout: xhrTimeout
                })
                    .done(function (data) {
        ...
        function clearAllCache() {
            if (hasLocalStorage()) {
                ...
                for (var i = localStorage.length - 1; i >= 0 ; i--) {
                    ...
                    if (key == 'undefined' || key.indexOf(prfx.app) !== -1) {
                ...
                (function(a) {
                    li.on('click', function () {
                        if ($(this).attr(dataAttr) !== '') {
							...
							//localStorage.setItem('defaultCity', '');			
                        } else {
                            locationId = null;
                    ...
                    Weather: [{
                        ...
                        url: function () {
                            // return EW.Helpers.protocol() +
                               return 'http://query.yahooapis.com/v1/public/yql?q={q}&format=json&_nocache={}&diagnostics=true&env=store://datatables.org/alltableswithkeys';
                        },
...
(function ( $ ) {
    ...
    var currentUnit = "C";
    var firstLoad = true;
    var extraDetailsOpen = false;
    var addLocationOpen = false;
    var searchInProgress = false;
    var default_city,ew,locationAdded;
    ...
    function date_time(id,firstload) {
        date = new Date;
        ...
        if(firstload){
            result = '<span class="clockTime"><span id="timetick">' + h + ':' + m + '</span><br><span id="weatherDivId"></span></span><span class="clockDate">' + days[day] + '</br>' + d + ' ' + months[month-1] +  ' </span>';
            try{
            ...
            try{
                ...
                document.getElementById("timetick").innerHTML = result;
            }catch(e){ }
        }
    ...
    function convertDegrees(val,unit) {
        if(unit == "C")
    ...
    function convertWind(val,unit){
        if(unit=="F")
            return (val / 1.609344).toFixed(2) + " mph";
        else
            return (val * 1.609344).toFixed(2) + " km/h";


    }
    function convertVisibility(val,unit){
        if(unit=="F")
            return (val / 1.609344).toFixed(2) + " miles";
        else
            return (val * 1.609344).toFixed(2) + " km";
    }
    ...
    function reloadHandler (data){
        ...
        ew = $('.weather-block').EasyWeather({
            ...
            load: function() {
                ...
                $('.default_city').html(default_city);
                $('.ew-left').attr('title',default_city);
                date_time('date_time',true);
                $(".ew-left").appendTo("#weatherDivId");



                if(!firstLoad) {

                    $('.weather_current_location').html("<span>" + ew[0].getLocation() + "</span>");
                    $('.default_city').html(ew[0].getLocation());
                    $('.ew-left').attr('title',ew[0].getLocation())
                }



                $('.ew-forecasts').append($("#extraContent").html());
                //Some code to perform when plugin finished loading
                if(lsExist() === true){
                    currentUnit = localStorage.getItem("nt_weather_unit");
                    if(typeof currentUnit == "undefined")
                        currentUnit = "C";

                    if(currentUnit == "F"){

                        unitChangeHandler($(".weather-f")[0]);
                    }


                    if(firstLoad){
                        //localStorage.removeItem("nt_weather_locations");
                        var locations = JSON.parse(localStorage.getItem("nt_weather_locations"));

                        if(locations){
                            for(var i =0;i<locations.length;i++){

                                $('.weather_locations').append("<li data-location='"+locations[i]+"'></li>");
                            }
                        }
                    }
                ...
                $('.weather-unit-menu').show();
                //fix points to always be centered
                $('.weather_locations').css({left:39-Math.round($('.ew-content .weather_locations li').length)+"%"});

                if(!firstLoad && locationAdded == true){

                    $('.weather_locations li').removeClass("active");
                    $('.weather_locations').append("<li class='active' data-location='"+ew[0].getLocation()+"'></li>");
                    locationAdded = false;

                    if(lsExist() === true){

                        var items = JSON.parse(localStorage.getItem("nt_weather_locations"));
                        if(!items)
                            items = [];
                        items.push(ew[0].getLocation());

                        localStorage.setItem("nt_weather_locations",JSON.stringify(items));

                    }

                }
                if(!firstLoad){
                    $('#weather_remove_location').show();
                    $('.ew-forecasts,.details,.ew-form').addClass('ew-visible');
                }

                $('.weather-block').on('ew.error', function(e, msg) {
                    if(msg =="Location not found."){

                        $('.ew-form').css({position:"initial"});
                        searchInProgress = true;
                    }
                });


                $("#weather_more_details").on("click",function(){
                    if(!extraDetailsOpen){
                        $('.weather_extra_details').delay(100).fadeIn().css("display","inline-block");
                        extraDetailsOpen = true;
                        $("#more_icon").removeClass("fa-angle-down").addClass("fa-angle-up");
                    }
                    else{
                        $('.weather_extra_details').fadeOut();
                        extraDetailsOpen = false;
                        $("#more_icon").removeClass("fa-angle-up").addClass("fa-angle-down");
                    }


                });

                $('.weather-block .ew-form').removeAttr('style').hide();

                $('.weather-block .weather_locations li:first-child').data("location",default_city);


                $('.weather-unit').on("click",function(){

                    if($(this).data("value") != currentUnit) {
                        unitChangeHandler(this);
                    }
                ...
                $('#weather_add_location').on("click",function(){

                    $('.weather-unit-menu').hide();
                    $('.ew-form').fadeIn(300);
                    $('.ew-input').focus().attr("placeholder","Enter City, e.g "+default_city).val("");
                ...
                //click handler for location changer buttons (the bullets in the bottom)
                $('.weather_locations li').off("click").on("click",function(e){
                    var loc = $(this).data("location");


                    $('.weather_locations li').removeClass("active");
                    $(this).addClass("active");
                    $('#extraContent .weather_locations').html($('.weather-block .weather_locations').html());

                    $('.ew-input').val(loc);
                    ew[0].setLocation(loc,true);
                    if($(this).index() > 0)
                        $('#weather_remove_location').show();
                    else
                        $('#weather_remove_location').hide();

                    $('.ew-forecasts,.details,.ew-form').addClass('ew-visible');
                    localStorage.setItem('defaultCity', loc);
                })  ;

                $('#weather_remove_location').on("click",function(){

                    var selector = $('*[data-location="'+$('.ew-content .weather_current_location span').html()+'"]');
                    var fallbackLocation = selector.prev().data("location");

                    if(fallbackLocation == "undefined" || typeof fallbackLocation == "undefined"){
                        if(lsExist())
                            localStorage.removeItem("nt_weather_locations");
                        location.reload();
                    }
                    ...
                    $('.weather_current_location').html("<span>"+fallbackLocation+"</span>");
                    ...
                    if(lsExist() === true){
                        ...
                        if(locations){
                            ...
                            for(var i =0;i<locations.length;i++){
                                ...
                                if(locations[i] != selector.data("location"))
                                    newLocations.push(locations[i]);

                            }
                            localStorage.setItem("nt_weather_locations",JSON.stringify(newLocations));
                        }
                    }
                    selector.remove();
                    $('.ew-input').val(fallbackLocation);
                    ew[0].setLocation(fallbackLocation,true);
            ...
            fnSetIconUrl: function(o){
                ...
                return setWeatherIcon(o.code)},
            showCountry: false,
            template: {
                current:
                '<div class="{cssprfx}wrap-cond click-block">'+
                '<span id="date_time"></span>' +
                '<div class="{cssprfx}left">'+
                '<div class="{cssprfx}temp">{temp}</div>'+
                '<i class="wi {icn}" alt="{icndesc}" title="{icndesc}" ></i>'+
                '</div>'+
                '<div class="{cssprfx}cont-info">'+
                '<div class="default_city"></div>'+
                '</div></div>'+
                '<div class="details">'+
                '<span id="weather_more_details" title="Show More Details"><i id="more_icon" class="fa fa-angle-down fa-2x" aria-hidden="true"></i></span>'+
                '<ul>'+
                '<li><span>Humidity</span>{Condition.humidity}</li>'+
                '<li><span>Rain</span>{Condition.precipitation}</li>'+
                '<li><span>Wind</span><p class="weather-wind">{Condition.wind}</p></li>'+
                '<li class="weather_extra_details"><span>Pressure</span>{Condition.pressure}</li>'+
                '<li class="weather_extra_details"><span>Visibility</span><p class="weather-visibility">{Condition.visibility}</p></li>'+
                '<li class="weather_extra_details"></li>'+
                '</ul>'+
                '</div>'+
                '<div class="{cssprfx}forecasts">{forecasts}</div>'+

                '<div class="{cssprfx}cont-details"></div>',

                forecast:
                '	<div class="{cssprfx}wrap-cond">'+
                '<div class="{cssprfx}date">{date}</div>'+
                '<div class="{cssprfx}img">'+
                '<i class="wi {icn}" alt="{icndesc}" title="{icndesc}" ></i>'+
                '</div>'+
                '<div class="{cssprfx}cont-info">'+
                '<div class="{cssprfx}min-max">{minmax}</div>'+
        ...
        $('.weather_current_location').html("<span>"+default_city+" (Current Location)</span>");
        ew[0].clearAllCache();

    }


    function unitChangeHandler(elem){

        currentUnit = $(elem).data("value");
        if(lsExist() === true){
            localStorage.setItem("nt_weather_unit",currentUnit);
        }
        $('.unit-active').removeClass("unit-active");
        $(elem).addClass("unit-active");

        $('.ew-content .ew-min,.ew-content .ew-max,.ew-temp').each(function(index,item){
            
            $(this).html(convertDegrees($(this).html().match(/\d+/)[0],currentUnit)+String.fromCharCode(176));

        });

        $('.weather-wind').html(convertWind(Number($('.weather-wind').html().replace(/[^0-9\.]+/g,"")),currentUnit));
        $('.weather-visibility').html(convertVisibility(Number($('.weather-visibility').html().replace(/[^0-9\.]+/g,"")),currentUnit));
    ...
    $.fn.weatherWidget = function() {
        ...
        // $.getJSON("http://ip-api.com/json/?callback=?", reloadHandler);
        $.getJSON("http://freegeoip.net/json", reloadHandler);




        $(document).on('click', '.click-block', function () {
            $('.ew-forecasts,.details').fadeIn(50,function(){
                $('.ew-forecasts,.details,.ew-form').addClass('ew-visible');
            });
        ...
        $(document).mouseup(function (e) {
            var container = $('.ew-visible');
            var searchForm = $('.ew-form,.ew-results');
            var searchResults = $('.ew-results');
            if (container.has(e.target).length === 0){
                $('.ew-forecasts').removeClass('ew-visible').hide();
                $('.details').removeClass('ew-visible').hide();
                $('.ew-form').removeClass('ew-visible').hide();
            }

            if(searchForm.has(e.target).length === 0 && addLocationOpen == true){
            ...
            if(searchResults.has(e.target).length === 0 && searchInProgress == true){

                searchInProgress = false;
                locationAdded = false;
                $('.ew-form').removeClass('ew-visible').hide();
                $('.weather-unit-menu').show();
                $('.ew-input').val(default_city);
                ew[0].setLocation(default_city,true);
