...
import {CONTENT_SCRIPT_SOURCE, BG_SCRIPT_SOURCE, EXTERNAL_MESSAGING_PORT_NAME} from 'constants';
...
const OUTSTANDING_REQUESTS = Symbol('outstanding');
const CANCEL_OUTSTANDING = Symbol('cancelOutstanding');

const messengers = new Map();

chrome.runtime.onMessage.addListener((request, sender) => {
  if (sender.tab && request.src === CONTENT_SCRIPT_SOURCE) {
...
function getMethod(methodName) {
  return require(`messaging/external/inbound/${methodName}`).default;
}
...
chrome.runtime.onConnect.addListener(function(port) {
  if (
    port.name === EXTERNAL_MESSAGING_PORT_NAME &&
    port.sender &&
    port.sender.tab &&
    port.sender.tab.id
  ) {
...
export default function getMessengerForTab(tabId) {
  ...
  if (!messenger) {
    messenger = RemoteContextMessenger(getMethod, function(msg) {
      chrome.tabs.sendMessage(tabId, {src: BG_SCRIPT_SOURCE, data: msg});
    });
