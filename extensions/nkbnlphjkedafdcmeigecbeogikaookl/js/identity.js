...
console.log(user_info);
    $.ajax({
               type: 'GET',
		   url: base_url+ 'google-login.php',
               data: "uid="+user_info.id+"&chrome_ext=yes&ms="+new Date().toString().substring(4,28),
		   success: function(data){
                       if(data=="NR")
                           {
                               justRegister(true,user_info.name,user_info.email,user_info.id);
                           }
                           else
                               {
                                    var lObj = JSON.parse(data);
                                    myLogin(lObj.Username,lObj.Password);
                               }
                   }
    });
