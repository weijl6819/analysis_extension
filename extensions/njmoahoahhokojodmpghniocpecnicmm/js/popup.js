...
var defaultQuery = getURLParameter("q");

var SETTING_NOTIFICATION = "isnotification";
function createFilterAllExt(ext){
 	
	return '<label><input type="checkbox" name="cbFilter" value="'+ext+'" >'+ext+'</label>';
}
...
function searchTheWeb(){
    ...
    var newURL = "http://apps.searchalgo.com/search/index.php?category=web&s=aaaa&cid=5323&q="+encodeURIComponent(query_search);
...
function filterByExt(filename){
	//Filter by ext// 
		...
		var cbAllDownload = document.getElementsByName("cbFilter");
		for(var i =0 ; i < cbAllDownload.length ; i++){
			if(cbAllDownload[i].checked)
			{
				filter.push(cbAllDownload[i].value);
			}
		}
		 
		if (filter.length > 0 && !( filter.indexOf( basename[basename.length-1])  > -1 )){
		   return true;
		}else{
			return false;
		}
...
function loadI18nMessages() {
  ...
  setProperty('#filter-all', 'title', 'FilterDownloadTitle');
  setProperty('#empty', 'innerText', 'zeroItems');
  setProperty('#searching', 'innerText', 'searching');
  
  setProperty('#search-zero-web', 'innerText', 'webSearchResults');
  setProperty('#search-zero-link', 'innerText', 'clickhere');
  ...
  setProperty('#clear-all', 'innerHTML', 'clearDownloads');
  setProperty('#settings', 'title', 'setting');
  setProperty('#btnSaveSetting', 'title', 'btnSaveSetting');
  setProperty('#itemsTitle', 'innerHTML', 'titleDownload');
  setProperty('#btnSaveSetting', 'innerText', 'SaveChanges');
  setProperty('#lblFolderDownload', 'innerText', 'cbFolderDownload');
  setProperty('#lblFolderDownload', 'title', 'cbFolderDownloadTitle');
  setProperty('#lblNotification', 'innerText', 'cbNotification');
  setProperty('#lblNotification', 'title', 'cbNotificationTitle');
  setProperty('#lblNewTab', 'innerText', 'cbNewTab');
  setProperty('#lblNewTab', 'title', 'cbNewTabTitle')
  setProperty('#settingTitle', 'innerText', 'settingTitle');
...
function DownloadItem(data) {
  ...
  if(item.state == "complete"){
  	item.getElement('icon').title = chrome.i18n.getMessage("openfile")
  }else{
  		item.getElement('icon').title = chrome.i18n.getMessage("erroropen")
  }
  item.getElement('icon').onclick = function() {
  	if(item.state == "complete"){
  		item.open();	
	}else{
		alert(chrome.i18n.getMessage("erroropen"));
	}
  ...
  [item.div, item.getElement('more')].concat(
    ...
    elem.onmouseover = function() {
      ...
      arrayFrom(items_div.children).forEach(function(other) {
      	//other.className = "itemOver";
      	
        if (other.item != item) {
          other.item.getElement('more').hidden = true;
         }else{
			
			other.classList.add('itemOver');
			other.onmouseout = function() {
				other.classList.remove('itemOver');
			}
		}
      });
...
DownloadItem.prototype.render = function(pageLoad) {
	
  var item = this;
  var now = new Date();
  var in_progress = (item.state == 'in_progress');
  var openable = (item.state != 'interrupted') && item.exists && !item.deleted;
//Fixed chrome bug after second open onchange not firing
if(in_progress && !item.paused && pageLoad){
 ...
 if(item.state == 'complete'){
  	if(item.danger == 'safe'){
		item.getElement('secureicon').src = "images/icons hover/safe.png";	
		item.getElement('secureicon').title = chrome.i18n.getMessage("safefile");	
	}else{
		item.getElement('secureicon').src = "images/icons hover/danger.png";	
		item.getElement('secureicon').title = chrome.i18n.getMessage("dangerfile");	
	}
  }
  ...
  var removedTxt =  chrome.i18n.getMessage('errorRemoved').toLowerCase();	
  if(item.state =="interrupted"){
  	removedTxt = chrome.i18n.getMessage('errorInterrupted').toLowerCase();
  }
  
  item.getElement('removed').innerText = item.basename +" ("+ removedTxt +")" ;
  ...
  function setByExtension(show) {
   ...
   try{
   	  var ext = item.basename.substr(item.basename.lastIndexOf('.') + 1);
...
DownloadManager.SaveChanges = function() {
	ext = "";
	 
	var cbNotification = document.getElementById('cbNotification');
	
	
	$("input[name=cbNotification]").each( function () {
		localStorage[SETTING_NOTIFICATION] = $(this).is(':checked');
	});
	$("input[name=cbNewTab]").each( function () {
		localStorage[SETTING_NEWTAB] = $(this).is(':checked');
	});
	$("input[name=cbFolderDownload]").each( function () {
		localStorage[SETTING_CUSTOMDOWNLOAD] = $(this).is(':checked');
	});
...
DownloadManager.settings = function() {
	 
	ext = "";
	 
	var obj = document.getElementById('headSettings');
	var headFilter = document.getElementById('headFilter');
	headFilter.hidden = true;
	
	obj.hidden = !obj.hidden;
	if(obj.hidden){
		document.getElementById('items').style.marginTop = "1em";
		$("#itemsTitle").show();
	} 
	...
	if(!settingOpened)
	{
	 ...
	 $( "#cbNotification").prop('checked', getNotificationSetting());
	 $( "#cbNewTab").prop('checked', getShowNewTabSetting());
	 $( "#cbFolderDownload").prop('checked', getCustomDownloadSetting());
	...
	var tableRef = document.getElementById('customDownloadTbl').getElementsByTagName('tbody')[0];
	...
	for(var i = 0; i < items.length; i++) {
	   ...
	   tableRef.innerHTML += '<tr><td><label><input type="checkbox" name="cbFolderDownloadItem" value="'+obj.type+'"  '+checked+' >'+obj.type+'</label></td><td>'+obj.path+'</td></tr>';

	}
  	
};

DownloadManager.showFilterAll = function() {
	
	ext = "";
	extArr = allExt.split(" ");
	for (var i = 0; i < extArr.length; i++) {
		if(extArr[i] != ""){
			ext += createFilterAllExt(extArr[i]);		
		}
	}
	
	headFilter.innerHTML = '<div id="filterTitle">'+ chrome.i18n.getMessage("FilterDownloadTitle")+'</div>';
	$("#itemsTitle").hide();
	headFilter.innerHTML += ext;
	var cbAllDownload = document.getElementsByName("cbFilter");
	for(var i =0 ; i < cbAllDownload.length ; i++){
		
		cbAllDownload[i].onchange=function(){
			//cbAllDown(this);
			DownloadManager.onSearch();
		};
	}
	var obj = document.getElementById('headFilter');
	
	obj.hidden = !obj.hidden;
	var headSettings = document.getElementById('headSettings');
	headSettings.hidden = true;
	if(obj.hidden){
		document.getElementById('items').style.marginTop = "1em";
		$("#itemsTitle").show();
	}else{
		document.getElementById('items').style.marginTop = "9em";
	}
...
DownloadManager.onSearch = function() {
  ...
  if (!query) {
    ...
    $("#itemsTitle").text(chrome.i18n.getMessage('titleDownload'));
  } else {
  	$("#itemsTitle").text(chrome.i18n.getMessage('titleSearching'));
    ...
    chrome.downloads.search({query: query}, function(results) {
      ...
      results.forEach(function(result) {
      	if(filterByExt(result.filename))
      	{
			return;
		}
      ...
      if(results.length == 0){
	  	$("#itemsTitle").text(chrome.i18n.getMessage('zeroItems'));	
	  }
...
if (chrome.downloads) {
  ...
  chrome.downloads.onChanged.addListener(function(delta) {
  	//chrome.runtime.sendMessage({downloadChanged:delta});
  ...
  window.onload = function() {
    ...
    document.getElementById('search-zero-link').onclick = searchTheWeb;
    
    
     document.getElementById('btnSaveSetting').onclick = function() {
        DownloadManager.SaveChanges();
      return false;
    };
    ...
    document.getElementById('filter-all').onclick = function() {
    	...
    	setTimeout(function(){DownloadManager.showFilterAll(); }, 200);
    ...
    document.getElementById('clear-all').onclick = function() {
    	if (confirm(chrome.i18n.getMessage("clearDownloadsConfirm"))) {
