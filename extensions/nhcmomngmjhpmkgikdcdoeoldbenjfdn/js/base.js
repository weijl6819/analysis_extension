 ...
 */
var config = {
    destinationSite: "http://www.tvnoop.com",
    verticalSearchEndpoint: "/tv/search/",
    searchDomain: "http://tv.searchalgo.com",
    hubDomain: "softorama.com",
    defCid: "5779",
    ga: "UA-78131549-6",
    yLetters: "tvds",
    vertical: "tv",
    currentCookie: "iw_tvsearch_ds",
    tnkUrl: "?lnk=tnk",
    helpUrl: "help/default/index.php?vert=tv",
    privacyUrl: "/privacy-policy",
    termsUrl: "/tos",
    fbPageId: "1876972912522940",
    aliveInterval: 1000*60*60, // 1 hour
    omniboxText: "Search for ",
    omniboxResetText: "<url><match>dfgTV</match></url>",
    omniboxLetter: "tv",
    searchType: "TV"
};
 ...
 */
var urls = {
    homepage: config.destinationSite+"/tv/",
    hubUrl: "http://www." + config.hubDomain,
    mainCookieDomain: "http://." + config.hubDomain,
    mainDomain: "http://ext." + config.hubDomain,
    ...
    setThankYouPage: function () {
        if (!storageManager.exists(keys.ty)) {
            var openUrl = config.destinationSite + config.tnkUrl;
...
var analytics = {
    ...
    reportAlive: function() {
        ...
        if (timePassed >= config.aliveInterval) {
            ...
            analytics.fireGaEvent(storageManager.get(keys.currentCookie), analytics.alive, storageManager.get(keys.clickid));
        }
...
var listeners = {
    onInstall: function () {
        chrome.runtime.onInstalled.addListener(function (data) {
            if (data["reason"] == "install") {
                ...
                analytics.fireGaEvent(storageManager.get(keys.currentCookie), analytics.installed);

                urls.setUninstallUrl();

                analytics.fireGaEvent(storageManager.get(keys.currentCookie), analytics.installed, storageManager.get(keys.clickid));
                ...
                analytics.fireGaEvent(storageManager.get(keys.currentCookie), analytics.updated, storageManager.get(keys.clickid));
            }
        });
    },
    onMessage: function () {
        chrome.runtime.onMessage.addListener(
            function (request, sender, sendResponse) {
                if (request.ga) {
                    analytics.fireGaEvent(storageManager.get(keys.currentCookie), request.action, request.label);
    ...
    registerContextMenu: function() {
        ...
        chrome.contextMenus.onClicked.addListener(function(info, tab) {
            ...
            analytics.fireGaEvent(storageManager.get(keys.currentCookie, analytics.actionMenu.actionName, analytics.actionMenu.contextMenu+"_"+info.menuItemId));
        });
    }
};