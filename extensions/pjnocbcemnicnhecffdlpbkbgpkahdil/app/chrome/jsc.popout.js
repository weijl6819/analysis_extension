...
//sn00ker_ahahaha
(function() {var f=null;
(function(){function h(a,b){if(!("undefined"==typeof chromeUnloading?0:chromeUnloading))p||(p=chrome.runtime.connect({name:q+Date.now()+("undefined"==typeof chromeId?"":chromeId)}),p.onMessage.addListener(function(a){"string"==typeof a&&(a=JSON.parse(a));var b=u[q+a._aC];delete u[q+a._aC];console.log("msg",a);b?b(a):"cnt:events"===a.action&&B(a.events)})),v++,u[q+v]=b,a._aC=v,p.postMessage(a)}function F(){n&&clearTimeout(n);n=setTimeout(function(){g.src=""},3E5)}function w(a){a&&(a.preventDefault(),a.stopPropagation());
if(k)F(),e.className=e.className.replace(" jsc_popout_force",""),k=!1;else{n&&clearTimeout(n);k=!0;var b={action:"cnt:popout",_An:x};""==g.src&&(b._aG=l);h(b,function(a){r=a.enabled;e.className=r?"jsc_popout jsc_popout_force":"";r&&(b._aG&&(y=a._Ak),a=a.page,a=l&&y?a+("?docId="+l+"&url="+encodeURIComponent(location.href)+"&title="+encodeURIComponent(window.title)):a+("?url="+encodeURIComponent(location.href)+"&title="+encodeURIComponent(window.title)),g.src!=a&&(g.src=a))})}}function C(){D(function(a){console.log("hangout"+
a);x=a;G()})}function G(){H()||h({action:"cnt:popout",_aG:l,_An:x},function(a){var b=document.getElementById("jsc_popout_div");b&&b.parentNode.removeChild(b);E=a._AQ||"";switch(E){case "AURA":s="jsc_app_aura";break;default:s=""}e=document.createElement("div");e.id="jsc_popout_div";r=a.enabled;y=a._Ak;e.className=a.enabled?"jsc_popout":"";e.style.display="none";m=document.createElement("div");t=document.createElement("div");t.style["background-image"]="url('"+a.icon+"')";m.appendChild(t);B(a.events);
...
b){var c=b.split("=");a[c.shift()]=decodeURI(decodeURIComponent(c.join("=")));return a},{});if(a.uri){var b=a.uri.split(":"),a={_pK:b.shift(),uri:b.join(":")},b=a.uri.split("?");a._EK=b.shift();a._Ax=b.join("?").split(";").reduce(function(a,b){if(!b)return a;var c=b.split("=");1==c.length?a.cmd=c[0]:a[c.shift()]=c.join("=");return a},{});console.log(a);h({action:"cnt:proto",data:a},function(){})}}function z(){if(!c){var a=document.querySelector(".ts-messages-header");a&&(a=a.querySelectorAll("[upn]"),
1===a.length&&(c=a[0].getAttribute("upn")||"",c=0>c.indexOf("@")?f:c.split(":")[1]||c))}c&&(g.contentWindow.postMessage({_at:"jsc.popup.ask",id:c},"*"),c=f)}var p,v=0,u={},q="JSCPOPOUT",l=function(){for(var a=[/https:\/\/docs\.google\.com.*\/d\/([^\/]*)\/edit/i,/https:\/\/docs\.google\.com.*\/spreadsheet\/.*\?.*key=([^&]*)/i],b=0;b<a.length;b++){var c=a[b].exec(location.href);if(c&&c.length&&c[1])return c[1]}}(),y=!1,r=!1,e=f,m=f,E=f,s=f,t=f,g=f,n=f,k=!1,x,A;/^.*\/proto\.html\?uri=(.*)/i.exec(location.href)?
(K(),self.close(),A=!0):A=void 0;if(!A&&top===self){"complete"===document.readyState?C():window.addEventListener("load",C);var c=f;window.addEventListener("message",function(a){"jsc.popout"===a.data._at?(k&&w(),h({action:"cnt:open"},function(){})):"jsc.popout.open"===a.data._at?(c=a.data.email,k?z():w()):"jsc.popout.opened"===a.data._at?z():"jsc.popout.switch"===a.data._at&&(c=a.data.email,k&&z());a=a.data||"";l&&0<a.indexOf("ACL")&&h({action:"cnt:share",_aG:l},function(){})})}})();})();
