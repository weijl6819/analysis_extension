...
/***/ function(module, exports, __webpack_require__) {
	...
	function SystemController()
	{
	    ...
	    var m_Version       = {};
	    var m_Plugin        = null;
	    this.cwicVersion = "11.8.1.250232";
	    ...
	    function onCwicInitialized(content)
	    {
	        ...
	        this.platformOS = (content.system || {}).platform || "windows";
...
/***/ function(module, exports, __webpack_require__) {
	...
	ActiveXControl.prototype.initialize = function()
	{
	    if ('ActiveXObject' in window)
	    {
	        ...
	        try
	        {
				...
				plugin.setClientId(this.cwicID);
				if (location.href.indexOf('ichrome-extension:') === 0) {
					plugin.setUrl('https://esna.communications.jabber');
					plugin.setHostName('esna.communications.jabber');
					plugin.setAppName('esna.communications.jabber');
				} else {
					plugin.setUrl(window.location.href);
					plugin.setHostName(window.location.hostname ? window.location.hostname : "file://");
					plugin.setAppName(window.document.title);
	...
	ActiveXControl.prototype.sendMessage = function(message)
	{
		...
		if (location.href.indexOf('ichrome-extension:') === 0) {
			message.client = {
				'id': this.cwicID,
				'url': 'https://esna.communications.jabber',
				'hostname': 'esna.communications.jabber',
				'name': 'esna.communications.jabber'
			};
		} else {
			message.client = {
				'id': this.cwicID,
	...
	function DockingWindow(windowType)
	{
	    ...
	    this.calculateNewPosition = function()
	    {
	        ...
	        if (('ontouchstart' in window) && (navigator.maxTouchPoints > 1))
	        {
	            ...
	            var scrollSizeX = currentFrame.document.width - currentFrame.innerWidth;
	            var scrollSizeY = currentFrame.document.height - currentFrame.innerHeight;
	        ...
	        {
	            position.top += Math.ceil(window.mozInnerScreenY);
	            position.left += Math.ceil(window.mozInnerScreenX);
	        }
