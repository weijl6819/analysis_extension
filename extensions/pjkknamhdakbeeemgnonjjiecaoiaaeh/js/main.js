...
function initSearchEngine() {
    chrome.cookies.get({"url": config.searchDomain, "name": "se"}, function (cookie) {
        if (cookie != undefined && cookie != null && cookie.value) {
            $("#seSelector").val(cookie.value);
        }
    });
    $("#seSelector").on('change', function () {
        storageManager.set(keys.searchEngine, $(this).val());
        gaReport(analytics.actionMenu.actionName, analytics.actionMenu.searchEngineSelected($(this).val()));
        chrome.runtime.sendMessage({se: $(this).val()}, function (response) {
