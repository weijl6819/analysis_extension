 ...
 */
 ...
 (function( $ ){
	...
	$.sliderTabs = function(container, options){
      ...
      $tabsListContainer.find('li a .remove_news').click(function() {
          var favs = JSON.parse(localStorage.getItem('news_fav'));
          var href_value = $(this).attr("term");
          for (var k in favs) {
              var knews = favs[k].knews;
              if (href_value == knews) {
                  favs.splice(k, 1);
              }
          }
          localStorage.setItem('news_fav', JSON.stringify(favs));
      ...
      $("<a href='#' class='ui-slider-plus-arrow' data-toggle='modal' data-target='#ModalNews'><div></div></a>").css({
				'width': settings.tabArrowWidth,
				'height': settings.tabHeight+2
			}).appendTo($tabsListContainer).click(function(e){
        $('#ModalNews').modal({
      ...
      if(direction=='left' && newMargin != 0)
        newMargin = newMargin - 33;
		...
		plugin.showTabArrows = function(){
			...
			$leftTabArrow.show();
			$rightTabArrow.show();
      var arr_width = parseInt(settings.tabArrowWidth) + 33;
