...
function adHide(div){
    ...
    if(ifm && ifm.length > 0 ){
        ...
        try {
            ...
            // console.log("Exp " + currfm.outerHTML.length );
            // if (exception instanceof DOMException && currfm.outerHTML.length > minLen
            //     && currfm.outerHTML.length < maxLimit){
            //     log('reddit', currfm.outerHTML);
            //     return true;
            // }
        }
        return false;
...
function log(domain, content){
    try {
        ...
        // console.log("content script send to log...");
