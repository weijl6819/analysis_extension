...
(function (angular) {
    ...
    angular.module('ReFigure', ['ngRoute', 'ngSanitize'])
        .constant('CookieToken', {
            COOKIE_URL: _gConst.EXTENSION_DOMAIN,
            COOKIE_DOMAIN: '.refigure.org',
            COOKIE_NAME: _gConst.AUTH_COOKIE,
            set: function (token) {
                chrome.cookies.set({
                    url: this.COOKIE_URL,
        ...
        .constant('STORAGE', {
            domain: _gConst.EXTENSION_DOMAIN,
            currentTab: null,
            rfFigures: null,
            foundFigures: null,
...
(function (angular) {
    ...
    Controller.$inject = ['$location', '$http', 'STORAGE', 'CookieToken'];

    function Controller($location, $http, STORAGE, CookieToken) {
...
(function (angular) {
    ...
    LoginController.$inject = ['AuthService', 'STORAGE'];

    function LoginController(AuthService, STORAGE) {
        var vm = this;
...
(function (angular) {
    ...
    Controller.$inject = ['$timeout', '$sce'];

    function Controller($timeout, $sce) {
        var exports = {
            loader: true,
            message: {
                text: '',
        ...
        function showWindow(params) {
            ...
            exports.window.content = $sce.trustAsHtml(params.content);
...
(function (angular) {
    ...
    Controller.$inject = ['$scope', '$window', 'AuthService', 'STORAGE', 'MessageService'];

    function Controller($scope, $window, AuthService, STORAGE, MessageService) {
        var vm = this;

        vm.logout = AuthService.logout;
        vm.store = STORAGE;
        vm.historyBack = historyBack;
        vm.showUpgradeInfo = showUpgradeInfo;
        vm.$onInit = activate;
        ...
        function showUpgradeInfo() {
            if (!vm.store.userInfo.Flags.UNFILTERED_EXTENSION) {
                // var href = vm.store.domain + '/upgrade';
                MessageService.showWindow({
                    content: '<div class="rf-window-content">' +
                            '    <div class="rf-window-title">' +
                            '       <img src="/img/refigure_logo_small.png" alt="Logo">' +
                                    'ReFigure' +
                            '    </div>' +
