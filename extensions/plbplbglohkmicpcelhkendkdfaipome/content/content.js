...
chrome.runtime.onMessage.addListener(function (request, sender, callback) {
    switch (request.type) {
        ...
        case _gConst.MSG_TYPE_START_SEARCH:
            refigure.isFiguresAddable = request.isFiguresAddable;
        ...
        case _gConst.MSG_TYPE_ADD_FIGURE_TO_COLLECTION:
            ...
            break;
        case _gConst.MSG_TYPE_SEARCH_OPEN_MARKER:
            var query = _gConst.PROCESSORS[request.key];
            if (query) {
                callback(typeof query === 'function' ? query() : Sizzle(query).length);
            } else {
                callback(false);
            }
            break;
...
function bindAddEvents() {
    Sizzle(parser.CONTENT_BLOCK_SELECTOR + ' img:not(.rf-not-addable)').forEach(function (el) {
...
parser = {
    ...
    parseFigures: function () {
        ...
        for (var i = 0; i < document.images.length; i++) {
            figures.push({
                ...
                Caption: document.images[i].alt || document.images[i].title
            });
        }
    ...
    getPageDOI: function () {
        ...
        if (metaDOI.length) {
            return metaDOI[0].content;
        } else {
            metaDOI = Sizzle('meta[name="dc.identifier"]');
            if (metaDOI.length) {
                return metaDOI[0].content.replace('doi:', '');
            }
        }
        window.logError('Article DOI not found');
        return '';
    },
...
angular.module('ReFigureContent', ['ngSanitize'])
    .constant('USER_INFO', {})
    ...
    .run(['$rootScope', '$http', '$q', function ($scope, $http, $q) {
        ...
        //--------REFIGURE-POPUP
        $scope.formData = {};
        $scope.saveCollection = function saveCollection(params) {
            if (!params.ID) {
                ...
                $http
                    ...
                    .then(function (resp) {
                        $scope.close();
                        $scope.formData = {};
                        if ($scope.userInfo.Flags.PRIVATE_REFIGURES) {
        ...
        //--------IMAGES-POPUP
        $scope.MAX_FIGURES_NUMBER = _gConst.MAX_FIGURES_PER_COLLECTION;
        $scope.collection = {};
        $scope.maxNumberExceeded = false;
        $scope.opts = {
            current: -1
        };
        $scope.exampleSrc = chrome.extension.getURL('img/select_image_example.png');
        $scope.logoSrc = chrome.extension.getURL('img/refigure_logo_small.png');

        $scope.$watch('collection.Figures.length', function (num) {
            var newStatus = !!(num && num >= _gConst.MAX_FIGURES_PER_COLLECTION);
            if (newStatus !== $scope.maxNumberExceeded) {
                $scope.maxNumberExceeded = newStatus;
                if ($scope.maxNumberExceeded) {
                    $scope.dialog = new Dialog();
                    $scope.dialog.buttons.cancel = false;
                    $scope.dialog.buttons.ok = 'CLOSE';
                    $scope.dialog
                        .show(
                            'You reached the max number of figures per ReFigure: ' +
                                $scope.MAX_FIGURES_NUMBER + '.<br>' +
                            'Figures are not addable anymore.'
                        );
                    window.unbindAddEvents();
                } else {
                    window.bindAddEvents();
                }
            }
        });
        ...
        $scope.remove = function (index) {
            $scope.dialog = new Dialog();
            $scope.dialog
                .show('Are you sure you would like to delete this image from refigure?')
                .then(function () {
                    $http
        ...
        $scope.addFigure = function (data) {
            if (data.ID) {
                ...
                if (window.refigure.isFiguresAddable) {
                    ...
                    $scope.dialog.buttons.cancel = false;
                    $scope.dialog.buttons.ok = 'CLOSE';
                    var href = _gConst.EXTENSION_DOMAIN + '/upgrade';
                    $scope.dialog.show(
                        'You need a <a href="' + href + '" target="_blank">full access license</a> ' +
                        'to add this picture to your ReFigure'
                    );
                }
            }
        };
        ...
        $scope.updateCollection = function () {
            getUserInfo()
                ...
                if ($scope.userInfo.Flags.PRIVATE_REFIGURES) {
                    ...
                    $http.put(_gApiURL + 'metapublication', params)
                        .then(function () {
        ...
        function showUpgradeInfo() {
            ...
            $scope.dialog.show(
                ...
                'To request early access, please email us at ' +
                '<a href="mailto:contactrefigure@gmail.com">contactrefigure@gmail.com</a>.'
            );
        }
    ...
    .directive('rfClearPaste', [function () {
        return {
            restrict: 'A', // only activate on element attribute
...
(function (self) {
    ...
    self.show = function (data, collection) {
        ...
        // if so, then use UPDATE not CREATE
    ...
    self.create = function () {
        ...
        domEl.innerHTML = '<div ng-include="\'view/image-popup.html\'"></div>';
...
(function (self) {
    ...
    self.create = function () {
        ...
        var domEl = document.createElement('div');
        domEl.innerHTML = '<div ng-include="\'view/refigure-popup.html\'"></div>';
        document.body.appendChild(domEl);
        angular.bootstrap(domEl, ['ReFigureContent'], {strictDi: true});
        _scope = angular.element(domEl).scope();
        return true;
