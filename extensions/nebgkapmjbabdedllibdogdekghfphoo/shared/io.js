...
if (typeof(_fpaJSIO) != 'boolean') {
	...
	// Example use:
	// var fileIn = _fpaFileIO.open('/test.txt');
	// if (fileIn.exists()) {
	// 	var fileOut = _fpaFileIO.open('/copy of test.txt');
	// 	var str = _fpaFileIO.read(fileIn);
	// 	var rv = _fpaFileIO.write(fileOut, str);
	// 	alert('File write: ' + rv);
	// 	rv = _fpaFileIO.write(fileOut, str, 'a');
	// 	alert('File append: ' + rv);
	// 	rv = _fpaFileIO.unlink(fileOut);
			...
			try {
				while (dirEntry.hasMoreElements()) {
					list.push(dirEntry.getNext()
									.QueryInterface(_fpaFileIO.localfileIID));
				}
				if (recursive) {
					var list2 = new Array();
